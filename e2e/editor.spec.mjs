...
test('traditional inline keyboard survives preview, Web handoff and Telegram send',async({page,context})=>{
  ...
  let sendBody;
  await telegramPage.route('**/api/send',async route=>{sendBody=route.request().postDataJSON();await route.fulfill({status:200,contentType:'application/json',body:JSON.stringify({ok:true,result:{message_id:151}})})});
  await telegram(telegramPage);await setEditor(telegramPage,'<p>Com teclado</p>');
  await telegramPage.evaluate(value=>{RMD.application.document().options.inlineKeyboard=value},keyboard);
  await telegramPage.evaluate(()=>__tg.MainButton.handler());await expect.poll(()=>sendBody).not.toBeUndefined();
