...
export async function telegramCall(token,method,body,{fetchImpl=fetch}={}){let response;try{response=await fetchImpl(`https://api.telegram.org/bot${token}/${method}`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(body)})}catch(error){if(error&&typeof error==='object')error.telegramResponse=false;throw error}let data;try{data=await response.json()}catch{const error=new Error(`Telegram retornou HTTP ${response.status} sem JSON válido`);error.telegramResponse=true;throw error}if(!response.ok||!data?.ok){const error=new Error(data?.description||`Telegram HTTP ${response.status}`);error.telegramResponse=true;throw error}return data}
