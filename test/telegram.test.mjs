...
test('enforces Telegram 32768-byte rich-message limit',()=>{assert.equal(validateRichHtml('a'.repeat(32768)).ok,true);assert.equal(validateRichHtml('a'.repeat(32769)).error,'payload_too_large');assert.equal(validateRichHtml('😀'.repeat(8193)).error,'payload_too_large')});
