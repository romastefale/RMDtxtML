...
test('release 1.0.0 keeps production boundaries and mandatory QA gate',async()=>{
  ...
  assert.match(docker,/COPY --from=test \/tmp\/rmdtxtml-qa-passed \/tmp\/rmdtxtml-qa-passed/);
  ...
  assert.match(index,/app\.css\?v=1\.0\.0/);
  assert.match(index,/app\.js\?v=1\.0\.0/);
