(()=>{
  ...
  async function claim(){
    ...
    try{
      ...
      localStorage.setItem(KEY,JSON.stringify({html:currentHtml(),rtl,skipEntityDetection}));
      updateStatus('Importado do Web');say('Composição transferida para o Telegram');
  ...
  async function openTelegram(){
    ...
    try{
      ...
      const result=await jsonResponse(response);
      localStorage.setItem(KEY,JSON.stringify({html:m.html,rtl,skipEntityDetection}));
